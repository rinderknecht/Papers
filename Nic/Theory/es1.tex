...
Let \mlp and \mrp be meta\-lexemes corresponding to the opening and
closing meta\-parentheses, whose concrete syntax is \texttt{\%(} and
\texttt{\%)}. These meta\-parentheses are inserted by the user in
order to guide matching the pattern against the parse tree by forcing
the enclosed pattern to match one subtree. Meta\-parentheses can be
present anywhere in the pattern as long as they are properly
paired. Unparsed patterns are formally defined as the smallest set
\(\overline{\cal P}\) such that
\begin{itemize}
...
\begin{figure}[b]
  ...
  \caption{A pattern tree}
  \label{es1_pattern_tree}
\end{figure}
\begin{remark}
...
\begin{itemize}
  ...
  \item \textbf{Binding.} The first element of the pattern is a
    ...
    (among all the premises) must be compatible with the binding \(x
