...
As a motivating example, let us consider a very simple
...
In the second approach to matching source code, code patterns are
...
%% Indeed, for either performance or historical reasons,
%% GLR parsers are very rarely used in existing tools. Porting an
%% existing parser to a different technology may be extremely difficult
%% (typically, the grammar has to be rewritten from scratch) and may
%% profoundly impact the rest of the tool, which is simply unaffordable
%% for most legacy tools.

\paragraph{Our Proposal.}

It is safe to say that there is no simple solution today for adding
...
%% The following
%% sections show that the mere idea of unparsing the AST must be cleverly
%% combined with other features in order to obtain a practical pattern
%% matching algorithm.

%% Some of the work reported here has been previously published as a
%% workshop paper~\cite{VolanschiRinderknecht:2008} which introduced
%% informally the main concepts of unparsed patterns. The topic and the
%% practical results are the same, but the present article differs on
%% many points, amongst which the following.
%% \begin{itemize}

%%   \item The approach is completely different. In the workshop paper,
%%     several algorithms for matching unparsed patterns were presented
%%     and compared to the classic (tree\hyp{}based) pattern
%%     matching. Here, only two algorithms are presented, \emph{both}
%%     matching unparsed patterns: one using backtracking and one with
%%     limited lexical lookahead, called \textit{ES(1)}. The former is
%%     proved to be sound and complete with respect to the general
%%     concept of close-tree matching (defined operationally), which is a
%%     special case of tree inclusion: this way, the backtracking
%%     algorithm can serve as a convincing reference for
%%     \textit{ES(1)}. So, strictly speaking, the properties in this new
%%     paper are different.

%%   \item This twofold presentation allows a didactic introduction to
%%     the usefulness of formal methods (some classic techniques in usage
%%     amongst programming language theorists) when building tools and
%%     systems.

%%   \item The formal logic is rigourous; there are no ellipsis in the
%%     arguments or formulae. The \textit{ES(1)} algorithm has been
%%     rewritten using only inference rules instead of guarded rewrite
%%     rules and separate recursive equations for the lookahead. The new
%%     version is not the same, because, for example, it fails sooner
%%     than \textit{ES(1)} in~\cite{VolanschiRinderknecht:2008} does, but
%%     the major improvements are compactness and structural induction on
%%     only one relation, allowing completely detailed proofs.

%%   \item We present two interfaces for using the pattern matchers
%%     internally in a compiler and we give some real runs of our
%%     prototype \Matchbox.
...
%%   \item We describe in detail our novel pattern matching based on
%%     unparsing and show its run\hyp{}time cost is linear in the size of
%%     the AST;

%%   \item we show how our pattern matching technology can be implemented
%%     in a completely language\hyp{}independent way;

%%   \item from a practical point of view, we expect that our approach
%%     will have an impact on existing parser\hyp{}based tools written in
%%     imperative languages, by providing them pattern matching at a
%%     minimal cost, as demonstrated by our very concise implementation
